...
<div class="form-bg">
  <form class="audit-form" @submit.prevent="OnSubmit">
    <label>¿LOS LEDS DEL MODEM ESTAN EN VERDE?</label>
    <select name="sltLedsVerdes" id="sltLedsVerdes" @change="LedsVerdes" v-model="valorLedsVerdes" required>
      <option value="">SELECCIONA LA RESPUESTA</option>
      <option value="SI">SI</option>
      <option value="NO">NO</option>
    </select>

    <label for="txtFotoModem" v-if="bandera">FOTO: MODEM FUNCIONANDO</label>
    <input type="file" accept="image/*" v-if="bandera" @change="onFileChangeModem">

    <label>¿SE INSTALO ROSETA?</label>
    <select name="sltInstalacionRoseta" id="sltInstalacionRoseta" @change="InstalacionRoseta" v-model="valorInstalacionRoseta" required>
      <option value="">SELECCIONA LA RESPUESTA</option>
      <option value="SI">SI</option>
      <option value="NO">NO</option>
    </select>

    <label v-if="bandera2">FOTO: ROSETA INSTALADA</label>
